...
if (isset($_POST['submit'])) {
    ...
    $status = isset($_POST['status']) ? htmlspecialchars($_POST['status']) : '1';
    ...
    if (!empty($idx)) {
        ...
        $dml = "UPDATE member SET firstname=?, lastname=?, email=?, gender=?, address=?, contact=?, type=?, status=? WHERE member_id=?";
        
        // Siapkan statement
        if ($stmt = mysqli_prepare($db, $dml)) {
            // Bind variabel ke statement sebagai parameter
            // Tipe data: s = string, i = integer
            mysqli_stmt_bind_param($stmt, "sssssssii", $firstname, $lastname, $email, $gender, $alamat, $phone, $tipe, $status, $idx);
        } else {
        ...
        $dml = "INSERT INTO member(firstname, lastname, email, gender, address, contact, type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        
        // Siapkan statement
        if ($stmt = mysqli_prepare($db, $dml)) {
            // Bind variabel ke statement sebagai parameter
            mysqli_stmt_bind_param($stmt, "ssssssss", $firstname, $lastname, $email, $gender, $alamat, $phone, $tipe, $status);
        } else {
